...
#h_m, e_m, a_m, i_m, raan_m, argp_m, ta_inf_m = hoe.hyperbolic_orbital_elements(r_mars + 100000, v_inf_mars, mu_mars, 0)
h_m, e_m, a_m, i_m, raan_m, argp_m, ta_inf_m = hoe.hyperbolic_orbital_elements(r_mars + 8000, v_inf_mars, mu_mars, 0)
...
# min = 10000000
# while i < len(x):
#     if np.linalg.norm([x[i], y[i], z[i]]) < min:
#         min = np.linalg.norm([x[i], y[i], z[i]])
#         p_index = i
#     i += 1

# print(x[p_index], y[p_index], z[p_index], dx[p_index], dy[p_index], dz[p_index])

y0 = [x[0], y[0], z[0], dx[0], dy[0], dz[0]]

t_span = [0, 5420]
t_eval = np.arange(0, 5420, 10)
...
#print(np.linalg.norm([dragpath.y[0][i], dragpath.y[1][i], dragpath.y[2][i]]) - r_mars)
...
h_e, e_e, a_e, i_e, raan_e, argp_e, ta_inf_e = hoe.hyperbolic_orbital_elements(r_earth + 57715, v_inf_mars, mu_earth, 0)
...
dragpathe = am.aero_capture(y0, t_span, t_eval, 2000, mu_earth, r_earth, 1)
...
#print(np.linalg.norm([x_m[0], y_m[0], z_m[0]]))
...
#ax.plot3D(x_j2, y_j2, z_j2, color='m', label='J_2 after 4 months')
