...
J0_E_d, T0_E_d = ot.julian_time_0(2032, 3, 17)          # depart Earth (01/07/2032)
J0_S_a, T0_S_a = ot.julian_time_0(2033, 6, 29)          # arrive Starman (23/06/2033)
J0_S_d, T0_S_d = ot.julian_time_0(2034, 12, 14)         # depart Starman (22/11/2033)
J0_M_a, T0_M_a = ot.julian_time_0(2035, 9, 29)          # arrive Mars (10/07/2034)
J0_M_d, T0_M_d = ot.julian_time_0(2035, 6, 6)         # depart Mars (08/11/2034)
J0_E_a, T0_E_a = ot.julian_time_0(2035, 12, 5)         # arrive Earth (24/10/2035)
print(J0_M_d, J0_E_a)
...
v_exc_S_a = V_tf1_a - V_S_a
v_exc_S_d = V_tf2_d - V_S_d

print("depart earth vinf: " + str(np.linalg.norm(v_exc_E_d)))
print("arrive Starman vinf: " + str(np.linalg.norm(v_exc_S_a)))
print("depart starman vinf: " + str(np.linalg.norm(v_exc_S_d)))
...
## rendezvous with starman, based on distance
R_StoS = np.zeros_like(x_tf1)
for i in range(len(R_StoS)):
    R_StoS[i] = np.linalg.norm(R_S_a - [x_tf1[i], y_tf1[i], z_tf1[i]])
rendezvous_point = np.where(R_StoS==min(R_StoS))[0]
...
##### HYPERBOLIC DEPARTURE/ARRIVAL TRAJECTORIES #####
...
fig = plt.figure()
ax = plt.axes(projection='3d')
ax.plot(x_E_d_hyp, y_E_d_hyp, z_E_d_hyp, "darkblue", linewidth=1, label="Departing Earth")
plot_planet("Earth")
plt.xlabel("X (km)")
plt.ylabel("Y (km)")
ax.set_zlabel("Z (km)")
...
fig = plt.figure()
ax = plt.axes(projection='3d')
ax.plot(x_M_a_hyp, y_M_a_hyp, z_M_a_hyp, "darkblue", linewidth=1, label="Arriving Mars")
plot_planet("Mars")
plt.xlabel("X (km)")
plt.ylabel("Y (km)")
ax.set_zlabel("Z (km)")
...
fig = plt.figure()
ax = plt.axes(projection='3d')
ax.plot(x_M_d_hyp, y_M_d_hyp, z_M_d_hyp, "darkblue", linewidth=1, label="Departing Mars")
plot_planet("Mars")
plt.xlabel("X (km)")
plt.ylabel("Y (km)")
ax.set_zlabel("Z (km)")
...
dv_E_d = vn_E_d_hyp[peri_E_d] - np.sqrt(mu_E/(6378+600))
dv_M_a = vn_M_a_hyp[peri_M_a] - np.sqrt(mu_M/(3390+400))
dv_M_d = vn_M_d_hyp[peri_M_d] - np.sqrt(mu_M/(3390+400))
dv_E_a = vn_E_a_hyp[peri_E_a] - np.sqrt(mu_E/(6378+600))
