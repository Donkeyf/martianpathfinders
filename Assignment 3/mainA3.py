...
# Time error
t_error = np.linspace(-T_Ep/2, T_Ep/2, 1000)

# Required delta v's
dv_req = np.zeros_like(t_error)
for i in range(len(t_error)):
    dv_req[i] = phase.phasing_deltav(T_Ep, r_Ep, mu_E, t_error[i])


t_error_10 = abs(t_error-600)
t_10_a = np.where(t_error_10 == np.min(t_error_10))[0]
t_error_m10 = abs(t_error+600.1)
t_10_b = np.where(t_error_m10 == np.min(t_error_m10))[0]

t_error_30 = abs(t_error-1800)
t_30_a = np.where(t_error_30 == np.min(t_error_30))[0]
t_error_m30 = abs(t_error+1800)
t_30_b = np.where(t_error_m30 == np.min(t_error_m30))[0]


plt.figure()
plt.plot(t_error, dv_req)

plt.plot(t_error[t_10_a], dv_req[t_10_a], "go", label="10 minutes ahead")
plt.plot(t_error[t_10_a], dv_req[t_10_a], "go")
plt.plot(t_error[t_30_a], dv_req[t_30_a], "ro", label="30 minutes ahead/behind")
plt.plot(t_error[t_30_b], dv_req[t_30_b], "ro")

plt.legend(fontsize=8)
plt.xlabel("Time Error (seconds)")
plt.ylabel("Delta-v required")
...
J0_E_d, T0_E_d = ot.julian_time_0(2041, 2, 1)          # depart Earth (01/07/2032)
J0_S_a, T0_S_a = ot.julian_time_0(2042, 5, 4)          # arrive Starman (23/06/2033)
J0_S_d, T0_S_d = ot.julian_time_0(2042, 5, 8)         # depart Starman (22/11/2033)
J0_M_a, T0_M_a = ot.julian_time_0(2043, 3, 17)          # arrive Mars (10/07/2034)
J0_M_d, T0_M_d = ot.julian_time_0(2043, 7, 15)         # depart Mars (08/11/2034)
J0_E_a, T0_E_a = ot.julian_time_0(2044, 6, 29)         # arrive Earth (24/10/2035)
...
R_S_d, V_S_d, h_S, e_S, a_S, T_S, n_S, i_S, raan_S, argp_S, ta_S, ma_S1 = fsv.find_orbital_elements(J0_S_d, "Starman")
R_M_a, V_M_a, h_M, e_M, a_M, T_M, n_M, i_M, raan_M, argp_M, ta_M, ma_M = fsv.find_orbital_elements(J0_M_a, "Mars")
R_M_d, V_M_d, h_M, e_M, a_M, T_M, n_M, i_M, raan_M, argp_M, ta_M, ma_M1 = fsv.find_orbital_elements(J0_M_d, "Mars")
R_E_a, V_E_a, h_E, e_E, a_E, T_E, n_E, i_E, raan_E, argp_E, ta_E, ma_E1 = fsv.find_orbital_elements(J0_E_a, "Earth")
...
v_exc_M_a = V_M_a - V_tf2_a                 # Entering Mars
v_exc_M_d = V_tf3_d - V_M_d                 # Escaping Mars
v_exc_E_a = V_tf3_a - V_E_a                 # Entering Earth

v_rel_S = V_tf1_a - V_S_a

print("relative velocity to Starman" + str(np.linalg.norm(v_rel_S)))
...
ax.plot(x_S, y_S, z_S, "hotpink", linestyle=":", linewidth=1.2, label="Starman")
ax.plot(x_M, y_M, z_M, "red", linestyle=":", linewidth=1.2, label="Mars")
...
ax.set_xlim3d(-2.5e8,2.5e8)
ax.set_ylim3d(-2.5e8,2.5e8)
ax.set_zlim3d(-2.5e8,2.5e8)

# ax.set_xlim3d(-2.5e8,-1.5e8)
# ax.set_ylim3d(-1e8,1e8)
# ax.set_zlim3d(-3e6,3e6)
...
h_E_d_hyp, e_E_d_hyp, a_E_d_hyp, i_E_d_hyp, raan_E_d_hyp, argp_E_d_hyp, ta_inf_E_d_hyp = ot.hyperbolic_orbital_elements(r_E+600, v_exc_E_d, mu_E, 1)

# define circular parking orbit based on hyperbolic trajectory
v_E_d_park, x_E_d_park, y_E_d_park, z_E_d_park, v_vec_E_d_park = ot.circular_parking_orbit(600, 0, i_E_d_hyp, raan_E_d_hyp, argp_E_d_hyp, mu_E)

# simulate hyperbolic trajectory
t_vec_E_d_hyp = np.linspace(0, 10000, 5000)
ta_E_d_hyp, ea_E_d_hype, ma_vec_E_d_hyp = ot.compute_hyperbolic_anomalies(t_vec_E_d_hyp, 0, e_E_d_hyp, h_E_d_hyp, mu_E)
...
# print("hyperbolic elements")
# print(e_E_d_hyp)
# print(np.rad2deg(i_E_d_hyp))
# print(np.rad2deg(raan_E_d_hyp))
# print(np.rad2deg(argp_E_d_hyp))
# print(np.array([dx_E_d_hyp[0], dy_E_d_hyp[0], dz_E_d_hyp[0]])-np.array([v_vec_E_d_park[0][0], v_vec_E_d_park[1][0], v_vec_E_d_park[2][0]]))
...
def check_vexc(v_exc):
    xexc = [0, 5000*v_exc[0]]
    yexc = [0, 5000*v_exc[1]]
    zexc = [0, 5000*v_exc[2]]
    plt.plot(xexc, yexc, zexc)
    return

# check_vexc(v_exc_E_d)

ax.plot(x_E_d_park, y_E_d_park, z_E_d_park, "red", linewidth=1, linestyle=":", label="Parking Orbit")
ax.plot(x_E_d_hyp, y_E_d_hyp, z_E_d_hyp, "red", linewidth=1, label="Hyperbolic Trajectory")
plot_planet("Earth")
...
# find orbital elements of hyperbolic trajectory
h_M_a_hyp, e_M_a_hyp, a_M_a_hyp, i_M_a_hyp, raan_M_a_hyp, argp_M_a_hyp, ta_inf_M_a_hyp = ot.hyperbolic_orbital_elements(r_M+400, v_exc_M_a, mu_M, 0)

# define circular parking orbit based on hyperbolic trajectory
v_M_a_park, x_M_a_park, y_M_a_park, z_M_a_park, v_vec_M_a_park = ot.circular_parking_orbit(400, 0, i_M_a_hyp, raan_M_a_hyp, argp_M_a_hyp, mu_M)

# simulate hyperbolic trajectory
t_vec_M_a_hyp = np.linspace(0, 10000, 5000)
ta_M_a_hyp, ea_M_a_hyp, ma_vec_M_a_hyp = ot.compute_hyperbolic_anomalies(t_vec_M_a_hyp, -np.pi, e_M_a_hyp, h_M_a_hyp, mu_M)
...
# # check_vexc(v_exc_M_a)

# ax.plot(x_M_a_park, y_M_a_park, z_M_a_park, "green", linewidth=1, linestyle=":", label="Initial Parking Orbit")
# ax.plot(x_M_a_hyp[:peri_M_a], y_M_a_hyp[:peri_M_a], z_M_a_hyp[:peri_M_a], "green", linewidth=1, label="Arriving Hyperbolic Trajectory")
# plot_planet("Mars")
...
# find orbital elements of hyperbolic trajectory
h_M_d_hyp, e_M_d_hyp, a_M_d_hyp, i_M_d_hyp, raan_M_d_hyp, argp_M_d_hyp, ta_inf_M_d_hyp = ot.hyperbolic_orbital_elements(r_M+400, v_exc_M_d, mu_M, 2)

# define circular parking orbit based on hyperbolic trajectory
v_M_d_park, x_M_d_park, y_M_d_park, z_M_d_park, v_vec_M_d_park = ot.circular_parking_orbit(400, 0, i_M_d_hyp, raan_M_d_hyp, argp_M_d_hyp, mu_M)

# simulate hyperbolic trajectory
t_vec_M_d_hyp = np.linspace(0, 10000, 5000)
...
print("hyperbolic elements")
print(np.rad2deg(i_M_d_hyp))
print(np.rad2deg(raan_M_d_hyp))
print(np.rad2deg(argp_M_d_hyp))
print(np.array([dx_M_d_hyp[0], dy_M_d_hyp[0], dz_M_d_hyp[0]])-np.array([v_vec_M_d_park[0][0], v_vec_M_d_park[1][0], v_vec_M_d_park[2][0]]))

# print("position")
# print([x_M_d_hyp[peri_M_d], y_M_d_hyp[peri_M_d], z_M_d_hyp[peri_M_d]])
# print("velocity hyperbolic")
# print([dx_M_d_hyp[peri_M_d], dy_M_d_hyp[peri_M_d], dz_M_d_hyp[peri_M_d]])
# print("velocity parking")
# print([v_vec_M_d_park[0][0], v_vec_M_d_park[1][0], v_vec_M_d_park[2][0]])
...
# check_vexc(v_exc_M_a)
# check_vexc(v_exc_M_d)

ax.plot(x_M_a_park, y_M_a_park, z_M_a_park, "green", linewidth=1, linestyle=":", label="Initial Parking Orbit")
ax.plot(x_M_a_hyp[:peri_M_a], y_M_a_hyp[:peri_M_a], z_M_a_hyp[:peri_M_a], "green", linewidth=1, label="Arriving Hyperbolic Trajectory")

ax.plot(x_M_d_park, y_M_d_park, z_M_d_park, "red", linewidth=1, linestyle=":", label="Departing Parking Orbit")
ax.plot(x_M_d_hyp, y_M_d_hyp, z_M_d_hyp, "red", linewidth=1, label="Departing Hyperbolic Trajectory")

plot_planet("Mars")

plt.title("Arriving/Departing Mars")
plt.xlabel("X (km)")
plt.ylabel("Y (km)")
ax.set_zlabel("Z (km)")
plt.legend(fontsize=8)
...
# find orbital elements of hyperbolic trajectory
h_E_a_hyp, e_E_a_hyp, a_E_a_hyp, i_E_a_hyp, raan_E_a_hyp, argp_E_a_hyp, ta_inf_E_a_hyp = ot.hyperbolic_orbital_elements(r_E+600, v_exc_E_a, mu_E, 0)

# define circular parking orbit based on hyperbolic trajectory
v_E_a_park, x_E_a_park, y_E_a_park, z_E_a_park, v_vec_E_a_park = ot.circular_parking_orbit(600, 0, i_E_a_hyp, raan_E_a_hyp, argp_E_a_hyp, mu_E)

# simulate hyperbolic trajectory
t_vec_E_a_hyp = np.linspace(0, 20000, 5000)
ta_E_a_hyp, ea_E_d_hyp, ma_vec_E_d_hyp = ot.compute_hyperbolic_anomalies(t_vec_E_a_hyp, -np.pi/2, e_E_a_hyp, h_E_a_hyp, mu_E)
...
# check_vexc(v_exc_E_a)

ax.plot(x_E_a_park, y_E_a_park, z_E_a_park, "green", linewidth=1, linestyle=":", label="Parking Orbit")
ax.plot(x_E_a_hyp[:peri_E_a], y_E_a_hyp[:peri_E_a], z_E_a_hyp[:peri_E_a], "green", linewidth=1, label="Hyperbolic Trajectory")
plot_planet("Earth")
plt.title("Arriving Earth")
plt.xlabel("X (km)")
plt.ylabel("Y (km)")
ax.set_zlabel("Z (km)")
plt.legend(fontsize=8)
...
# circular parking orbits, no aerocapture
...
print("dv to to go from Earth parking to escape: " + str(dv_E_d))
print("dv to to go from hyperbolic to Mars parking: " + str(dv_M_a))
print("dv to to go from Mars parking to escape: " + str(dv_M_d))
print("dv to to go from hyperbolic to Earth Parking: " + str(dv_E_a))




### J2 Perturbations in Mars Parking ###

J = 1960.45e-6      # J2 constant for Mars
raan_dot = -((3*np.sqrt(mu_M)*J*r_M**2) / (2*(1-0**2)**2*(r_M+400)**(7/2))) * np.cos(i_M_a_hyp)
raan_M_a_hyp = raan_M_a_hyp + raan_dot*(J0_M_d-J0_M_a)*60*60*24

v_M_a_parkJ2, x_M_a_parkJ2, y_M_a_parkJ2, z_M_a_parkJ2, v_vec_M_a_parkJ2 = ot.circular_parking_orbit(400, 0, i_M_a_hyp, raan_M_a_hyp, argp_M_a_hyp, mu_M)

h1=np.cross([x_M_a_parkJ2[0], y_M_a_parkJ2[0], z_M_a_parkJ2[0]], [v_vec_M_a_parkJ2[0][0], v_vec_M_a_parkJ2[1][0], v_vec_M_a_parkJ2[2][0]])
h2=np.cross([x_M_d_park[0], y_M_d_park[0], z_M_d_park[0]], [v_vec_M_d_park[0][0], v_vec_M_d_park[1][0], v_vec_M_d_park[2][0]])

rot_angle = np.arccos( np.dot(h1,h2) /  np.linalg.norm(h1)/np.linalg.norm(h2) )

dv_planechange = 2*v_M_a_parkJ2*np.sin(rot_angle/2)
print(np.rad2deg(rot_angle))
print("plane change dv: " + str(dv_planechange))

# plot trajectory
fig = plt.figure()
ax = plt.axes(projection='3d')
plot_planet("Mars")
ax.plot(x_M_a_park, y_M_a_park, z_M_a_park, "limegreen", linewidth=1, label="Initial Parking Orbit")
ax.plot(x_M_a_parkJ2, y_M_a_parkJ2, z_M_a_parkJ2, "blue", linewidth=1, label="Post J2 Parking Orbit")
ax.plot(x_M_d_park, y_M_d_park, z_M_d_park, "red", linewidth=1, label="Departure Parking Orbit")
plt.xlabel("X (km)")
plt.ylabel("Y (km)")
ax.set_zlabel("Z (km)")
plt.legend(fontsize=8)
